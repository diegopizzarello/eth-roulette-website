...
const Header = ({ status, account, connect }: HeaderProps) => {
  const onboarding = new MetaMaskOnboarding({
    forwarderOrigin: "roulette.pizzarello.dev",
  });
  ...
  const AccountInfo = ({ address }: AccountInfoProps) => (
    <AccountContainer>
      ...
      />
      <Text style={{ color: "white", maxWidth: 120 }} strong ellipsis>
        {account}
      </Text>
    </AccountContainer>
  );
  ...
  return (
    <Container>
      ...
      <Title
        level={3}
        style={{ color: "#F49309", letterSpacing: 2.2, marginBottom: 0 }}
      >
        ROULETTE
      </Title>
      {status === "not_installed" && (
        <Button type="primary" shape="round" onClick={onClickInstall}>
          Install Metamask
        </Button>
      )}
      {status === "installed" && (
        <Button type="primary" size="large" shape="round" onClick={connect}>
          Connect Metamask
        </Button>
      )}
      {(status === "connected" || status === "wrong_network") && account && (
