...
const contractAddress = "0x2517243630EC27FA5603F2274a3FCB2E3A6a07d3";
...
const Game = () => {
  ...
  const rouletteContract = useContract<WaveRoulette>(
    contractAddress,
    contractABI
  );
  ...
  const onNewWave = (from: string, timestamp: BigNumber, message: string) => {
    ...
    setWaves((prevState) => [newWave, ...prevState]);
    setPlayers((prevState) => [newWave, ...prevState]);
  };
  ...
  const onNewWinner = (
    ...
    const winner = roulettePlayers.findIndex(
      (player) => player.waver === from && player.message === message
    );
    setRouletteResult({ winner, players: roulettePlayers });
    setPlayers([]);
    setWaves((prevState) => [lastWave, ...prevState]);
  };
